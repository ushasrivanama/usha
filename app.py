...
def predict(bed,bath,loc,size,status,face,Type):
    ...
    input_data=np.array([[bed,bath,selected_location_numeric,size,
    selected_status_numeric,
    selected_direction_numeric,
    selected_property_type_numeric]])
    ...
    return prediction
selected_location_numeric=location_mapping['Poranki']
print(f'------------{(selected_location_numeric)}----------')
